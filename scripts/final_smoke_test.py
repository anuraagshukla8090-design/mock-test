"""Final smoke test on real MinerU data."""
...
full_opts = sum(1 for q in qs if q.section_type == "mcq" and len(q.options) == 4)
partial   = [q.question_number for q in qs if q.section_type == "mcq" and len(q.options) < 4]

print(f"Total : {total}  MCQ: {mcq}  Integer: {integer}  Answered: {answered}")
print(f"MCQ with 4/4 options  : {full_opts}/{mcq}")
print(f"MCQ with <4 opts      : {partial}")
print()
for q in qs[:20]:
    stem_preview = q.stem_md[:60].replace("\n", " ") if q.stem_md else "EMPTY"
    print(f"Q{q.question_number:2d} [{q.section_type:7s}] opts={len(q.options)} ans={repr(q.answer)[:8]}  {stem_preview}")
